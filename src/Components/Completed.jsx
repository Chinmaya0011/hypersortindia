...
const Pending = () => {
  ...
  useEffect(() => {
    ...
    if (filteredTasks.length > 0) {
      filteredPendingTasks = filteredTasks.filter(task => task.taskStatus === 'Completed');
    } else if (priorityFilteredTasks.length > 0) {
      filteredPendingTasks = priorityFilteredTasks.filter(task => task.taskStatus === 'Completed');
    } else {
      filteredPendingTasks = tasks.filter(task => task.taskStatus === 'Completed');
    }
  ...
  return (
    <div className={style.pending}>
      <div className={style.board}>
        <h1 className={style.heading} style={{ backgroundColor: 'green' }}>Completed</h1>
