...
const Pending = () => {
  ...
  useEffect(() => {
    ...
    if (filteredTasks.length > 0) {
      filteredPendingTasks = filteredTasks.filter(task => task.taskStatus === 'Deployed');
    } else if (priorityFilteredTasks.length > 0) {
      filteredPendingTasks = priorityFilteredTasks.filter(task => task.taskStatus === 'Deployed');
    } else {
      filteredPendingTasks = tasks.filter(task => task.taskStatus === 'Deployed');
    }
  ...
  return (
    <div className={style.pending}>
      <div className={style.board}>
        <h1 className={style.heading} style={{ backgroundColor: 'orange' }}>Deployed</h1>
