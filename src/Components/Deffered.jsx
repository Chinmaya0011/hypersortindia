...
const Pending = () => {
  ...
  useEffect(() => {
    ...
    if (filteredTasks.length > 0) {
      filteredPendingTasks = filteredTasks.filter(task => task.taskStatus === 'Deferred');
    } else if (priorityFilteredTasks.length > 0) {
      filteredPendingTasks = priorityFilteredTasks.filter(task => task.taskStatus === 'Deferred');
    } else {
      filteredPendingTasks = tasks.filter(task => task.taskStatus === 'Deferred');
    }
  ...
  return (
    <div className={style.pending}>
      <div className={style.board}>
        <h1 className={style.heading} style={{ backgroundColor: 'red' }}>Deferred</h1>
